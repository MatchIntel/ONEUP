...
1. Delete the old files from the GitHub repository, then extract this ZIP and upload every file from inside it directly to the repository root. Do not upload a containing `1UP-Railway-Website` folder.
...
- Player photos use the exact original `https://unavatar.io/x/{handle}` browser-loading method from `1UP V1.html`, including lazy loading and the no-referrer policy. The avatar itself links directly to that player's X profile.
